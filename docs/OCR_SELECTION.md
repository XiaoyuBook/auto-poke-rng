...
评测日期：2026-09-23。本次只确定模型并保存评测方法，没有接入脚本 OCR、搜图标签或应用界面。
...
6. 检测评测显式使用官方 `inference.yml` 的 mean/std 与各代 DB 阈值。使用 `limit_type=max` 避免小 ROI 被默认的短边 736 放大成数千像素宽。此处图片均小于 960px；RapidOCR 的 max 模式在更大输入上会动态选边长，后续接入仍需自行设置输入尺寸上限。
