...
主进程按 4,096 帧分批计算，原始 Seed、Offset、存档和筛选条件保持一致，以每批的初始帧续算。结果超过 250,000 条时明确报错并清空本次结果，要求缩小范围或增加筛选，不返回截断的成功结果。表格采用连续滚动的虚拟列表，仅渲染可见行及上下各 8 行缓冲，没有分页；完整结果仍保存在内存中，复制和 CSV 导出涵盖本次全部结果。浏览器预览和引擎错误不会产生模拟数据。
...
点击结果单元格后可直接输入数字或字母，在该列全部结果中按显示文本的前缀定位，不区分字母大小写。连续按键在 400 毫秒内累积，超过则开始新查找；新查找从当前行的下一行开始，累积输入从当前行继续，末尾回绕，无匹配时保持原位置。这个交互参考 Qt `QAbstractItemView::keyboardSearch`；本应用使用固定 400 毫秒，未读取 Qt 在 Windows/macOS 上采用的系统间隔。切换单元格、离开表格、切换列显示或能力值显示会重置输入。Backspace 可删去末尾字符，Esc 清空查找，方向键移动单元格，Home/End 定位首尾，PageUp/PageDown 按可视高度移动。命中行会自动滚入视野，当前单元格有边框标记，状态栏显示查找词与结果位置。
...
`tests/rng-client-regressions.cjs` 覆盖跨批次真实结果、默认搜索范围、取消/重试、窗口销毁、来源限制、结果上限及计算器与搜索取消的隔离。前端测试覆盖菜单、存档编辑与持久化、存档传参、版本限制、真实结果展示、零上限、错误、完整结果复制、列隐藏与对齐、计算器输入与过期响应。`src/components/StaticResultsTable.test.tsx` 覆盖虚拟滚动、大小调整、跨原分页边界的前缀定位、超时与重置、回绕、无匹配、大小写、当前列、能力值切换和键盘事件作用域。

`tests/pokefinder-electron.cjs` 验证真实 Electron preload/IPC/原生进程链路，并用 100,000 条真实结果验证滚动首尾、滚动条定位中间行、身高 `255` 与十六进制键盘定位、可见单元格位置和 DOM 行数上限。定点页面、菜单、存档管理、表格设置、个体值计算器和定位截图保存至 `node_modules/.tmp/pokefinder-review/`。
