...
4. 模板与 ROI 均有效时，眨眼捕获页面的视频角落会显示实时匹配分数与可编辑的识别阈值，画面上显示红色 ROI 框和绿色匹配眼睛框；分数落入眨眼阈值区间时两框变黄。离开该页面后这些叠加内容会隐藏。可再用“识别预览”单独核对，或直接点“捕捉 Seed”“校正”“TID/SID 测种”。阈值是 OpenCV `TM_CCOEFF_NORMED` 的相关系数，不是百分比置信度。
5. 视频下方保留固定日志区。左侧“捕捉 Seed”与“TID/SID 测种”并排作为主按钮，“校正”与“Timeline”并排；捕捉时的眨眼计数显示在底部运行提示。捕获完成后持续推进，视频左上角从上到下显示当前帧数、Seed0、Seed1，Seed 保留本次捕获的基准值，当前帧数随推进更新。该信息只在眨眼捕获页面显示，框选时暂时隐藏。玩家模式可点击“Timeline”，沿用原版倒计时后切换模型；“停止”、断源或切换游戏终止推进。1 PK NPC 校正成功后依原版直接进入 Timeline，宝可梦 NPC 按原版设为 1。
6. 两段 64 位 Seed 可在视频左上角直接选中文字复制；捕获后四段 32 位状态自动用于“校正”。尚无有效状态时，“校正”会提示先捕捉 Seed 或选择已有配置。Seed 全程使用十六进制字符串。

配置名称可以直接输入中文；右侧下箭头可选择已有配置，“新增”清空为默认配置，“保存”按当前名称保存在本地，同名会更新，最多保留 20 份。高级时序参数在左侧直接显示，无需展开。模板 PNG、像素 ROI、源分辨率、阈值、NPC 数、自动获得的校正状态及所有高级时序字段随配置保存；旧配置自动补入时序默认值。“浏览”读取原版 JSON 的 `view/thresh/white_delay/advance_delay/advance_delay_2/npc/timeline_npc/pokemon_npc/reident_1_pk_npc`，以及配置目录或其上一级目录中的相对 PNG 模板。导入不会修改采集设备；没有连接视频、ROI 越界或存在原版 crop 裁剪时需重新框选。导入后点“保存”保存在本软件内，不覆盖原 JSON。分辨率变化后需要重新框选；换源时不把未完成的截图选择应用到新源。框选按源像素存储，支持黑边、不同宽高比、拖动缩放以及反向框选。
...
主进程测试覆盖 IPC 来源、参数范围、取消重试、启动失败、旧事件隔离及断源。前端测试覆盖 BDSP 专属入口、右侧视频／日志保留、中文配置名称与下拉选择、新增和保存、旧配置搜索范围归一化、运行状态、黑边坐标换算和 Seed 字符串传递。模式切换后底部运行提示显示所选模式的 40／7／20／64 次目标，避免沿用其他模式的进度。

Electron 测试启用已有“测试彩条（模拟）”，验证实际 C++ 共享帧 → Python/OpenCV → IPC → UI 的预览链路、右键拖动框选和双框显示，并确认没有重开视频源。检查 1500、1280 和 1100 宽窗口布局：日志固定在视频下方，左侧两排操作按钮可见，展开高级参数不横向裁切，停止按钮始终可达；使用独立的前端结果样本检查窄窗口视频左上角的完整 Seed，不把该样本当作捕获结果验证。截图在 `node_modules/.tmp/blink-review/`。这不是 Switch 实机眨眼录像验证；实机成功率仍需用实际场景、模板与采集卡验证。
