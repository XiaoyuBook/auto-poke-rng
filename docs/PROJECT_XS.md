...
3. 点击“框选眼睛区域”，同样在视频上右键拖动圈出覆盖眼睛移动范围的 ROI，松开即保存。ROI 的宽高都不能小于眼睛模板；若新框选不符合要求，会提示并保留框选状态供重试，原配置不被覆盖。反过来，新截取的眼睛若大于已有 ROI，也会要求缩小眼睛框或先扩大 ROI。导入的旧配置尺寸不匹配时，先重新框选才能开始识别。视频右键菜单仍提供这两个框选入口。
...
5. 视频下方保留固定日志区。左侧“捕捉 Seed”与“TID/SID 测种”并排作为主按钮，“校正”与“Timeline”并排；捕获或校正过程中，当前按钮变为“停止”，其他模式按钮禁用。捕捉进度显示在视频右上角阈值下方；左侧不再保留底部状态栏，配置与识别错误等反馈只在操作区按需出现。求解完成后按钮恢复原文案，进入持续推进；顶部状态显示当前帧数，视频右上角不再显示推进状态或停止入口。此时启动其他捕获或校正任务会先结束旧推进。视频左上角从上到下显示当前帧数、Seed0、Seed1，Seed 保留本次捕获的基准值，当前帧数随推进更新。该信息只在眨眼捕获页面显示，框选时暂时隐藏。玩家模式可点击“Timeline”，沿用原版倒计时后切换模型；断源或切换游戏终止推进。1 PK NPC 校正成功后依原版直接进入 Timeline，宝可梦 NPC 按原版设为 1。
