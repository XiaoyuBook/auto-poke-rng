...
function pack(version, body) {
  ...
  const manifest = { schemaVersion: 1, id: 'bdsp-demo', name: 'BDSP 官方脚本包', description: '测种、过帧和撞闪脚本与配套图像标签。', instructions: '使用前请核对游戏画面与运行起点。', game: 'BDSP', authors: ['XiaoyuBook'], version, minimumAppVersion: '0.1.0', installFolder: 'BDSP',
    files: Object.entries(contents).map(([name,bytes]) => ({ path: name, bytes: bytes.length, sha256: hash(bytes) })) };
...
app.whenReady().then(async () => {
  ...
  registerScriptRepository({ ipcMain, getMainWindow: () => main, rootDirectory: scripts, userData: app.getPath('userData'), appVersion: '0.1.0', gate,
    ...
    fetch: async url => new Response(url.endsWith('catalog.json') ? JSON.stringify(remote.catalog) : remote.bytes),
  ...
  await click('检查更新');
  await until(`document.querySelector('.repository-detail')?.textContent.includes('v1.1.0')`, 'new version loaded');
  await click('预览更新');
  ...
  await click('导入 ZIP');
  await until(`document.querySelector('.repository-detail')?.textContent.includes('v1.2.0')`, 'zip import preview');
