...
def test_bundled_roamer_scripts_use_runner_wait_mode(script_name, target_species):
    script_dir = Path(__file__).resolve().parents[2] / "third_party/bdsp-automation-reference/script"
...
def test_bundled_shaymin_script_does_not_repeat_flash_frame_wait():
    script_path = Path(__file__).resolve().parents[2] / "third_party/bdsp-automation-reference/script" / "谢米.txt"
