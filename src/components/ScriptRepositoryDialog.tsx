  ...
  return <Dialog title="脚本仓库" className="script-repository-dialog" close={() => { if (!pending.current) close(); }}>
    ...
    <div className="repository-body" aria-busy={busy}>
      ...
      <section className="repository-browser" aria-label="仓库脚本包">
        ...
        <div className="repository-filters" aria-label="脚本包筛选">{[['all','全部'], ['installed','已安装'], ['updates','有更新']].map(([value,label]) => <button key={value} aria-pressed={filter === value} disabled={busy} onClick={() => { setFilter(value); setFileFilter('all'); setPlan(null); }}>{label}</button>)}</div>
      ...
      <section className="repository-detail" aria-label="脚本包详情">
        ...
        <div className="repository-detail-content" ref={detailScroll}>
          {settings ? <div className="repository-settings"><h2>仓库设置</h2><p className="muted">选择适合当前网络的更新渠道，或手动导入下载好的脚本包。</p>
            ...
            {tab === 'guide' ? <div className="repository-guide" role="tabpanel" aria-label="使用说明">
              ...
              <div className="repository-file-categories"><button aria-pressed={fileFilter === 'all'} onClick={() => setFileFilter('all')}>全部</button>{fileCategories.map(category => <button key={category} aria-label={'文件分类：' + category} aria-pressed={fileFilter === category} onClick={() => setFileFilter(category)}>{category}</button>)}</div>
              <p className="muted">以下为实际安装文件名。脚本与配套资源作为完整包一起安装。</p>
              {visibleFiles.length ? <table><thead><tr><th>文件名称</th><th>用途</th><th>大小</th></tr></thead><tbody>{visibleFiles.map(file => <tr key={file.path}><td>{fileIcon(file.path)}<span title={file.path}>{file.path}</span></td><td>{categoryOf(file)}</td><td>{fileSize(file.bytes)}</td></tr>)}</tbody></table> : <p>{loadingDetails ? '正在读取文件清单…' : '此分类暂无文件。'}</p>}
            </div>}
        ...
        <footer className="repository-footer"><span>{selectedCategory && !settings && !plan ? `${shown.filter(item => repositoryCategory(item) === selectedCategory).length} 个脚本包` : current && !settings && !plan ? `${files.filter(file => /\.(txt|rng)$/i.test(file.path)).length} 个脚本 · ${new Set(files.map(categoryOf)).size} 类文件` : '官方脚本仓库'}</span><span>个人修改受保护 · 安装后可离线使用</span></footer>
      </section>
    </div>
  </Dialog>;
}
