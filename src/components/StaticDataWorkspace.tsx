...
export function generateStaticResults(params: SearchParams): StaticResult[] {
  ...
  for (let advances = 0; advances < end; advances += 1) {
    ...
    state = pidValue;
    const nature = NATURES_ZH[pidValue % NATURES_ZH.length];
    const ivs = statLabels.map((_, index) => {
      state = nextValue(state);
      return (state >>> ((index * 3) % 24)) % 32;
    }) as [number, number, number, number, number, number];
    const shinyValue = (((pidValue >>> 16) ^ (pidValue & 0xffff)) & 0xfff) < 8;
    const generatedShiny = shinyValue ? ((pidValue & 1) === 0 ? 'Star' : 'Square') : '否';
    const shiny = params.target.shiny === '锁闪' ? '否' : generatedShiny;
    const ability = params.target.ability === '0/1' ? String(pidValue % 2) : params.target.ability === '隐藏' ? '2' : params.target.ability;
    const gender = params.target.genderMode === 'genderless'
      ? '-'
      : params.target.genderMode === 'female'
        ? '雌'
        : params.target.genderMode === 'male'
          ? '雄'
          : ((pidValue & 1) ? '雌' : '雄');
    const genderFilterValue = gender === '-' ? '无性别' : gender === '雄' ? '雄性' : '雌性';
    const height = (state >>> 8) & 0xff;
    const weight = (state >>> 16) & 0xff;
    if (!params.skipFilter) {
      if (params.nature !== '任意' && params.nature !== nature) continue;
      if ((params.shiny === '异色' && shiny === '否') || (params.shiny === '非异色' && shiny !== '否') || (params.shiny === 'Star' && shiny !== 'Star') || (params.shiny === 'Square' && shiny !== 'Square')) continue;
      if (params.ability !== '任意' && params.ability !== ability) continue;
      if (params.gender !== '任意' && params.gender !== genderFilterValue) continue;
    ...
    rows.push({ advances, ec: ecValue.toString(16).toUpperCase().padStart(8, '0'), pid: pidValue.toString(16).toUpperCase().padStart(8, '0'), shiny, nature, ability, gender, ivs, stats, height, weight, characteristic: getCharacteristicForState(ecValue, ivs) });
...
export function StaticDataWorkspace({ onLog }: { onLog?: (message: string) => void }) {
  const [category, setCategory] = useState<StaticTarget['category'] | 'all'>('starters');
  ...
  const targetOptions = useMemo(() => getStaticTargets(category, 'BDSP'), [category]);
  const target = targetOptions[targetIndex] ?? targetOptions[0];
    ...
    <div className="static-settings-content">
      ...
      <section className="static-config-card" aria-label="定点数据参数">
        ...
        <div className="static-config-column" aria-label="定点设置"><header><Database size={14} /><h3>设置</h3></header>
          <label>分类<select aria-label="分类" value={category} onChange={event => { setCategory(event.target.value as StaticTarget['category'] | 'all'); setTargetIndex(0); }}>{CATEGORY_OPTIONS.map(option => <option key={option.key} value={option.key}>{option.label}</option>)}</select></label>
          <label>宝可梦<select aria-label="宝可梦" value={targetIndex} onChange={event => setTargetIndex(Number(event.target.value))}>{targetOptions.map((item, index) => <option key={`${item.speciesKey}-${item.version}`} value={index}>{item.species} [{getCategoryLabel(item.category)}]{item.roamer ? ' 游走' : ''}</option>)}</select></label>
        ...
        <div className="static-config-column static-filter-column" aria-label="筛选项"><header><Filter size={14} /><h3>筛选项</h3></header>
          <div className="static-filter-layout">
            ...
            <div className="static-other-filters">
              ...
              <label>性格<select aria-label="性格筛选" value={nature} onChange={event => setNature(event.target.value)}><option>任意</option>{NATURES_ZH.map(item => <option key={item}>{item}</option>)}</select></label>
              ...
              <label className="static-check"><input type="checkbox" checked={skipFilter} onChange={event => setSkipFilter(event.target.checked)} />取消筛选</label><label className="static-check stats-toggle"><input type="checkbox" checked={showStats} onChange={event => setShowStats(event.target.checked)} />显示能力值</label><button className="static-link-button" type="button" disabled>个体值计算器</button>
            </div>
          </div>
        </div>
      </section>
